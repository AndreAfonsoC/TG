...
if not resultado_calibracao['success']:
    raise RuntimeError("A calibração do motor falhou. Verifique os parâmetros de entrada.")

print("\nMotor calibrado com sucesso!")
print(f"  - T04 de Projeto: {resultado_calibracao['optimal_t04']:.2f} K")
print(f"  - Vazão de Ar de Projeto (nível do mar): {resultado_calibracao['optimal_mass_flow_rate']:.2f} kg/s")
print(f"  - Empuxo de Projeto verificado: {resultado_calibracao['final_thrust_kN']:.2f} kN")
print(f"  - TSFC de Projeto verificado: {resultado_calibracao['final_tsfc']:.5f} kg/(s*kN)")
...
perfil_de_voo = [
    ...
    {'name': 'Decolagem',      'duration_min': 1,  'altitude_ft': 0,    'mach': 0.2, 'thrust_percentage': 100},
    {'name': 'Subida 1',       'duration_min': 8,  'altitude_ft': 5830, 'mach': 0.298,'thrust_percentage': 1},
    {'name': 'Subida 2',       'duration_min': 8,  'altitude_ft': 17500,'mach': 0.494,'thrust_percentage': 1},
    {'name': 'Subida 3',       'duration_min': 8,  'altitude_ft': 29170,'mach': 0.691,'thrust_percentage': 1},
    {'name': 'Cruzeiro',       'duration_min': 10,'altitude_ft': 35000,'mach': 0.789,'thrust_percentage': 1},
...
if missao.results:
    ...
    print(f"Combustível Total Necessário: {missao.results['Combustível Inicial (kg)']:.2f} kg")
